...
export function LiveInterviewAgent({ attemptId, companyName, role, questionPrompt }: { attemptId?: string, companyName?: string, role?: string, questionPrompt?: string }) {
    ...
    const handleEndInterview = async () => {
        ...
        try {
            ...
            // Build display transcript in original chronological order, with user and AI turns interleaved.
            const formattedTranscript = formatInterviewTranscript(logs);
            const hasUserLogs = logs.some((log) => log.role === 'user');
            const userSection = pcmTranscript && !hasUserLogs ? `You: ${pcmTranscript}` : '';
            const answerToSave = [formattedTranscript, userSection].filter(Boolean).join('\n\n') || answerForEvaluation;
